...
export function ProjectCard({ project, onClick }: ProjectCardProps) {
  ...
  return (
    ...
    >
      <Tilt3D className="h-full w-full" intensity={9} scale={1.02}>
        <button
          type="button"
          onClick={() => onClick?.(project)}
          className="group relative block h-full w-full overflow-hidden rounded-2xl border border-border bg-surface text-left transition-colors duration-300 hover:border-accent hover:shadow-[0_28px_60px_-28px_rgba(255,107,157,0.55)]"
          style={{ transformStyle: "preserve-3d" }}
        >
          <div
